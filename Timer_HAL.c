...
void InitHWTimers() {
    ...
    Timer32_initModule(TIMER32_0_BASE, TIMER0_PRESCALER, TIMER32_32BIT, TIMER32_PERIODIC_MODE); //TIMER0_PRESCALER == TIMER32_PRESCALER_1
...
//void Init1sTimer() {Timer32_initModule(TIMER32_1_BASE,     TIMER32_PRESCALER_1, TIMER32_32BIT, TIMER32_PERIODIC_MODE);
...
void StartOneShot3sTimer() {
    //Timer32_initModule(TIMER32_0_BASE, TIMER0_PRESCALER, TIMER32_32BIT, TIMER32_PERIODIC_MODE); //TIMER0_PRESCALER == TIMER32_PRESCALER_1
    Timer32_setCount(TIMER32_0_BASE, 9000000);   //  3s period
    Timer32_startTimer(TIMER32_0_BASE, true);
}

int OneShot50msTimerExpired(){
    return (Timer32_getValue(TIMER32_0_BASE) == 0);
}

void StartOneShot50msTimer() {
    Timer32_setCount(TIMER32_0_BASE, 150000);   //  50ms period maybe add another 00
    Timer32_startTimer(TIMER32_0_BASE, true);
}

int OneShot2500msTimerExpired(){
    return (Timer32_getValue(TIMER32_1_BASE) == 0);
}

void StartOneShot2500msTimer(){
    Timer32_setCount(TIMER32_1_BASE, 75000000); // 2500ms period, should double check
